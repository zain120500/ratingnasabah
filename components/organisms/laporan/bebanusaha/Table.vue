<template>
    <EasyDataTable hide-footer :server-items-length="bebanusaha.reportbebanusaha.total" border-cell alternating
        theme-color="#1e73be" :loading="bebanusaha.loading" table-class-name="pgi-table" :headers="headers"
        :items="bebanusaha.reportbebanusaha.data">
        ...
        <template #item-created="{ created_at, created_by }">
            <div>
                <div>Oleh : {{ created_by }}</div>
    ...
    <PaginateContainer>
        <div class="flex items-center gap-x-5 justify-between text-[14px]">
            ...
            <div>
                {{ bebanusaha.reportbebanusaha.from }} - {{ bebanusaha.reportbebanusaha.to }} dari {{
                    bebanusaha.reportbebanusaha.total }}
            </div>
        </div>
        <v-pagination v-model="bebanusaha.reportbebanusaha.current_page" :pages="bebanusaha.reportbebanusaha.last_page"
...
const bebanusaha = useReportBebanUsahaStore()
await bebanusaha.getReportBabanUsaha()
